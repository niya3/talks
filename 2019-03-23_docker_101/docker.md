...
1. Больше ты не программист локалхоста. "На моей машине всё работает" больше не работает - держи докерфайл, который демонстрирует ошибку.
...
* Каждый шаг сборки генерирует один слой.
  * Слой содежит файлы(или мета-информацию), которые изменились после выполнения текущей инструкции
...
Аналогия с историей git: слой - комит, слои образуют дерево. В корне дерева специальный пустой образ `scratch`. Образ - состояние рабочей директории для некоторой ревизии.
...
```
Проверям что образ появился, создаём и запускаем контейнер.
...
* Сборка с явным yказанием образов, с которых надо взять кэш: `docker build --cache-from=name1 --cache-from=name2`, **но**
...
`build` - создать новый образ из локального докерфайла или удалённого. **NB**  `git://` только для [ssh по 22 порту](https://github.com/moby/moby/pull/33711#issuecomment-373807568)
...
`save/load` запаковать/распаковать образ как tar-архив с метаинформацией, может пригодиться для переноса на удалённую машину в обход реестра.

**NB** с каким тэгом запакуешь, с таким и распакуешь

**NB** `save image1:x image2:y image3:z` создаcт один архив меньшего размера, чем три команды (если есть общая история)
...
* `--rm` удалить контейнер по заверешнии процесса
* `-ti` запустить в интерактивном режиме, подключиться к stdin
* `-p 8080:80` пробросить порт с хостовой машины на хост контейнера
...
1. Адреса не резолвятся во время сборки/в контейнере: [Broken DNS](https://docs.docker.com/install/linux/linux-postinstall/#specify-dns-servers-for-docker)
